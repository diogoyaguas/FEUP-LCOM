      ...
      switch (_ENDPOINT_P(msg.m_source)) {
        case HARDWARE:
          if (msg.NOTIFY_ARG & irq_set) {
            if (sys_inb(OUT_BUF, &key) != OK) return -1;

            if (key == ESC_KEY) stop = -1;
          }
        default:
          break;
      }
...
int handler_c_poll(unsigned char *scancode) {
  ...
  for (tries = 0; tries < 5; tries++) {
    if (sys_inb(STATUS_PORT, &status) != OK) continue;  // try again
    ...
    if (status & BIT(0)) {
      if (status & (PARITY | TIMEOUT)) {  // parity and timeout detection
        ...
        if (sys_inb(OUT_BUF, &readoutbuf) != OK) {
          return -1;
...
int kbd_poll() {
  ...
  while (scancode != ESC_KEY) {
    ...
    // Fail
    if (handler_errorlevel == -1) return -1;

    // Successfully read, print
    if (handler_errorlevel == 0) write_code(scancode);
  }
  ...
  // read_outb_cmd(OUT_BUF, KBD_CMD_BUF);
  // read_outb_cmd(byte | OBF, OUT_BUF);

 ...
 *
 * Tests drawing a (filled) square with the specified size and color, at the
 * specified
 *  coordinates (which specify the square center with respect to the center
 *  of the screen) in video mode 0x105
 *
 * Calls video_dump_fb() immediately after writing the sprite to the
 * frame-buffer.
 *
 ...
 *
 * Tests drawing a line segment with the specified end points and the input
 * color,
 *  by writing to VRAM in video mode 0x105
 *
 * Calls video_dump_fb() immediately after writing the line to the frame-buffer.
 *
 * @param xi horizontal coordinate of "first" endpoint, starts at 0 (leftmost
 * pixel)
 * @param yi vertical coordinate of "first" endpoint, starts at 0 (top pixel)
 * @param xf horizontal coordinate of "last" endpoint, starts at 0 (leftmost
 * pixel)
 ...
 * @param xi horizontal coordinate of upper-left corner, starts at 0 (leftmost
 * pixel)
 ...
 *
 * Tests moving a XPM on the screen along horizontal/vertical axes, at the
 * specified speed and frame-rate, in video mode 0x105
 *
 * If speed is positive, then it specifies the displacement in pixels between
 * consecutive frames
 * If speed is negative, then it specifies the number of frames required for a 1
 * pixel displacement
 *
 * Calls video_dump_fb() every time it copies the XPM to the frame buffer.
 *
 * @param xpm array with XPM (assuming indexed color mode)
 * @param xi initial horizontal coordinate of upper-left corner, starts at 0
 * (leftmost pixel)
 * @param yi initial vertical coordinate of upper-left corner, starts at 0 (top
 * pixel)
 * @param xf final horizontal coordinate of upper-left corner, starts at 0
 * (leftmost pixel)
 * @param yf final vertical coordinate of upper-left corner, starts at 0 (top
 * pixel) *
...
int video_test_move(char *xpm[], unsigned short xi, unsigned short yi,
                    unsigned short xf, unsigned short yf, short s,
                    unsigned short f);

/**
 * @brief Tests retrieving VBE controller information  (VBE function 0x0)
 *
 * Tests retrieving VBE controller information (VBE function 0x0) and its
 * parsing.
 * Upon its invocation it should display the following pieces information, one
 * per line:
 * 	- VbeVersion
 * 	- List of mode numbers (in hexadecimal) supported, using ':' as a
 * separator
 * 	- Total memory in KB
 *
 * @return 0 upon success, non-zero upon failure

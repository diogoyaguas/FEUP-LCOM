...
void *vg_init(unsigned short mode) {
  ...
  mr.mr_base =
      (phys_bytes)
          vbe_mode.PhysBasePtr;  // depois trocar por generico (endereco base)
...
void doubleBufferToVideoMem() { memcpy(video_mem, double_buffer, vram_size); }

void bitmapDrawer(Bitmap *bmp, int x, int y) {
  drawBitmap(bmp, x, y, double_buffer);
}

void bitmapdrawerBackground(Bitmap *bmp, int x, int y) {
  drawBitmap_background(bmp, x, y, double_buffer);
}
